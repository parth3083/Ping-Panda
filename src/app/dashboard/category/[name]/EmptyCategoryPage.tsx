...
function EmptyCategoryPage({ categoryName }: { categoryName: string }) {
  const router = useRouter();
  const { data } = useQuery({
    queryKey: ["category", categoryName, "hasEvents"],
    queryFn: async () => {
      const res = await fetch("/api/polling-category", {
        method: "GET",
        headers: {
    ...
    >
      <h2 className="text-xl/8 font-medium text-center tracking-tight text-gray-950">
        Create your first {categoryName} event
          ...
          <p className="text-sm/6 text-gray-600 mb-8 max-w-md text-center overflow-hidden">
              Get started by sending a request to our tracking API:</p>
